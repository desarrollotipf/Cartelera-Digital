                    ...
                    >
                      <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: hasImage ? '100%' : 'auto' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: hasImage ? '0.75rem' : '0.4rem' }}>
                          <div className="hr-icon-circle bday-avatar-animated" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            {item.icon && (item.icon.startsWith('http') || item.icon.startsWith('/') || item.icon.startsWith('data:')) ? <img src={item.icon} alt="Icono" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '14px' }} loading="lazy" /> : <Pin size={24} color="#e11d48" strokeWidth={2.3} />}
                          </div>
