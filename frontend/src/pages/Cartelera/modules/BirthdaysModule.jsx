  ...
  return (
    ...
    >
      ...
      <div className="bday-stage-container" style={todayBirthdays.length === 0 ? { gridTemplateColumns: '1fr' } : {}}>
        {/* COLUMNA LATERAL (30%): Cumpleaños del Mes */}
        <div className="bday-weekly-sidebar" ref={bdayGridRef}>
          ...
          <div className="bday-weekly-list" style={todayBirthdays.length === 0 ? { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.25rem', alignContent: 'start' } : {}}>
            {birthdays.length === 0 ? (
              <div style={{ padding: '3rem 1rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '1.15rem', fontStyle: 'italic', gridColumn: '1 / -1' }}>
              ...
              birthdays.map((w, i) => {
                ...
                return (
                  <div key={w.id || i} className="bday-month-card stagger-card-pop" style={{ '--idx': i, position: 'relative', display: 'flex', flexDirection: 'column', gap: '0.3rem', alignItems: 'flex-start' }}>
